...
class Music(commands.Cog):
    ...
    async def add_queue(self, interaction: discord.Interaction, url: str):
        ...
        if interaction.user.voice and interaction.user.voice.channel == self.voice_client.channel:
            ...
            try:
                name = get_music_name(url)
            except Exception as e:
                await interaction.response.send_message(f"Failed to get audio source for {url}.\nPlease ensure you entered a Youtube URL")
                return
            self.music_queue.append(url)
            await interaction.response.send_message(f"Added {name} to the music queue.")
    ...
    async def show_queue(self, interaction: discord.Interaction):
        ...
        try:
            embed.add_field(name="currently playing", value= get_music_name(self.current_track))
            for i, url in enumerate(self.music_queue, start=0):
                embed.add_field(name=f"Song {i}", value=get_music_name(url), inline=False)
    ...
    async def play_music(self):
        if self.music_queue:
            ...
            song_url = self.current_track
            audio_url = get_audio_source(song_url)
            audio_source = FFmpegPCMAudio(audio_url, before_options="-reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 5")
            self.voice_client.play(audio_source, after=lambda e: self.bot.loop.create_task(self.play_music()))
...
def get_music_name(url):
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        info = ydl.extract_info(url, download=False)  # Set download to False to only get info
        return info.get('title', 'Unknown Title')  # Get the title from the extracted info
